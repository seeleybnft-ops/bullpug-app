...
/**
 * Derive a darker (factor > 0) or lighter (factor < 0) hex variant of a
 * base color. Used by `SculptedPugBody` so each skin's brow/wrinkle/sheen
 * accents are computed from its body hue instead of hardcoding 10 sets.
 */
...
/**
 * SculptedPugBody — single sculpted procedural pug shared across every
 * non-skeletal skin. Replaces the legacy low-poly sphere stack so every
 * Bullpug — Guardian, Ethereal, Diamond, Gold, Silver, Heatmap, Radioactive,
 * Zombie, Aqua, Inferno, Cyber — gets the same chunky pug silhouette.
 *
 * Per-skin styling comes through `sk` (the skin's visual lookup):
 *   • `sk.body / sk.belly / sk.dark` drive body, underside, ear/paw/snout
 *     colors. Brow shelf and 3 brow wrinkles auto-derive as darker
 *     shades of `sk.body` via `shade()`.
 *   • `sk.metalness >= 0.6` flips materials into a polished metal look
 *     (clearcoat 0.5, sheen off, envMapIntensity 1.2) — Gold, Silver,
 *     Cyber.
 *   • `sk.emissiveIntensity > 0.3` activates glowing eyes — Radioactive,
 *     Fire, Ethereal, etc — and skips the white catch-light spec dots
 *     since the eyes self-light.
 *   • Diamond (`skinId === "diamond"`) gets a full `meshPhysicalMaterial`
 *     transmission pass on the body (IOR 2.4, attenuation, transparent)
 *     so light actually refracts through it.
 *   • Cyber (`sk.extra === "robot"`) gets the canonical roof antenna.
 *
 * The nose pad, snout crease, mouth crease, nostril dots, tongue tip,
 * catch-lights and horns are skin-agnostic so the "pug" read stays
 * consistent across every variant.
 *
 * Shape spec (same as Iteration 153/154 Guardian — slimmed haunch):
 *   • Pear-shaped barrel (chest sphere + mid belly + slim haunch),
 *     belly highlight, chest tuft, shoulder bumps.
 *   • Brachycephalic head (flattened cranium + cheek bulges + brow
 *     shelf + 3 stacked wrinkles + heavy hanging jowls + flat pushed-in
 *     snout + nose / nostrils / mouth / tongue + deep-set eyes + floppy
 *     oversized ears + canonical bull horns).
 *   • Stubby legs with 4-toe paws.
 *   • Double-coil curly tail (`CatmullRomCurve3` + `tubeGeometry`).
 */
...
export function Bullpug({ refY, refX, sliding, running, shieldActive, skinId = "default", idle = false }) {
  ...
  return (
    <group ref={root} rotation={[0, Math.PI, 0]}>
      ...
      {/* Surface-level texture overlay (fur, fractals, fire cracks, etc) */}
      <SkinSurface skinId={skinId} sk={sk} />
...
/**
 * Procedurally-placed fur tuft. A small tapered cone tinted to the skin's
 * fur color, scattered across the body surface (top of head, back, sides)
 * to give base-skin pugs actual hair texture instead of looking like
 * smooth balloons.
 */
function FurTuft({ position, rotation, color, size = 0.05 }) {
  return (
    <mesh position={position} rotation={rotation}>
      <coneGeometry args={[size * 0.4, size * 2.4, 6]} />
...
/**
 * Surface-level texture overlay rendered between the base body mesh and
 * the SkinExtras VFX layer. Gives each skin a distinct touch:
 *  - Default/Guardian, Aqua, Phantom: fur tufts (real little hairs on
 *    the head/back, density and color tuned per skin).
 *  - Diamond: crystalline faceted shell (Icosahedron wireframe).
 *  - Inferno: lava cracks seeping through the body (emissive, animated).
 *  - Heatmap: thermal hot-patch dots pulsing red→yellow.
 *  - Gold/Silver: metallic facet pinpoints.
 *  - Radioactive: glowing green vein patches.
 *  - Zombie: torn-flesh patches in darker green.
 *  - Cyber: dark armor seams + small LED dots.
 */
function SkinSurface({ skinId, sk }) {
  ...
  useFrame((state) => {
    const t = state.clock.elapsedTime;
    // Inferno lava cracks pulse like real seeping fire
    if (lavaRef.current) {
      lavaRef.current.children.forEach((child, i) => {
        if (child.material) {
          const phase = i * 0.7;
          child.material.emissiveIntensity = 1.6 + Math.sin(t * 5 + phase) * 1.0 + Math.sin(t * 11 + phase * 1.3) * 0.5;
        }
      });
    }
    // Heatmap thermal patches breathe slower, all in sync
    if (thermalRef.current) {
      const pulse = 1.2 + Math.sin(t * 2.4) * 0.8;
      thermalRef.current.children.forEach((child) => {
        if (child.material) child.material.emissiveIntensity = pulse;
      });
    }
    // Diamond fractal shell rotates very slowly
    if (fractalRef.current) {
      fractalRef.current.rotation.y += 0.004;
      fractalRef.current.rotation.x += 0.0015;
    }
  });

  // ── Inferno (fire) — lava cracks seeping through the body
  if (skinId === "fire") {
    // 10 emissive lava patches scattered around the body sphere using
    // tiny stretched ellipsoids that look like glowing cracks at distance.
    const cracks = [
      { p: [-0.30, 0.62, 0.30], r: [0.0, 0.5, 0.2], s: [0.06, 0.012, 0.12] },
      { p: [0.32, 0.58, 0.28], r: [0.1, -0.4, 0.3], s: [0.05, 0.012, 0.10] },
      { p: [-0.25, 0.45, 0.40], r: [0.3, 0.6, -0.2], s: [0.04, 0.010, 0.09] },
      { p: [0.28, 0.40, 0.35], r: [-0.2, -0.5, 0.4], s: [0.05, 0.011, 0.11] },
      { p: [0.0, 0.68, 0.45], r: [0.5, 0, 0.0], s: [0.07, 0.013, 0.13] },
      { p: [-0.36, 0.55, 0.0], r: [0.0, 1.2, 0.5], s: [0.05, 0.010, 0.09] },
      { p: [0.36, 0.50, -0.05], r: [0.1, -1.2, -0.4], s: [0.06, 0.012, 0.10] },
      { p: [0.0, 0.35, -0.45], r: [0.0, 3.1, 0.0], s: [0.06, 0.010, 0.10] },
      { p: [-0.20, 0.72, -0.20], r: [0.2, 2.4, 0.3], s: [0.05, 0.011, 0.08] },
      { p: [0.20, 0.30, 0.42], r: [-0.3, 0.4, -0.5], s: [0.04, 0.010, 0.09] },
      // Head cracks
      { p: [-0.18, 1.18, 0.80], r: [0.4, 0.5, 0.2], s: [0.04, 0.010, 0.08] },
      { p: [0.18, 1.12, 0.78], r: [0.3, -0.5, -0.2], s: [0.04, 0.010, 0.08] },
    ];
    return (
      <group ref={lavaRef}>
        {cracks.map((c, i) => (
  ...
  // ── Heatmap — thermal hot-patches pulsing red→orange→yellow
  if (skinId === "heatmap") {
    const patches = [
      { p: [-0.28, 0.65, 0.30], c: "#FFE26B", em: "#FF6B35" },
      { p: [0.30, 0.60, 0.30], c: "#FFB066", em: "#FF4A00" },
      { p: [-0.10, 0.78, 0.40], c: "#FFE26B", em: "#FFB066" },
      { p: [0.18, 0.42, 0.38], c: "#FF8C42", em: "#FF3A00" },
      { p: [-0.32, 0.48, 0.0], c: "#FFB066", em: "#FF6B35" },
      { p: [0.32, 0.52, -0.10], c: "#FF8C42", em: "#FF3A00" },
      { p: [0.0, 0.78, -0.30], c: "#FFE26B", em: "#FF6B35" },
      { p: [-0.18, 1.10, 0.78], c: "#FFB066", em: "#FF4A00" },
      { p: [0.18, 1.16, 0.78], c: "#FFE26B", em: "#FF6B35" },
    ];
    return (
      <group ref={thermalRef}>
        {patches.map((p, i) => (
          <mesh key={i} position={p.p}>
            <sphereGeometry args={[0.07, 10, 8]} />
            <meshStandardMaterial color={p.c} emissive={p.em} emissiveIntensity={1.4} toneMapped={false} />
          </mesh>
        ))}
      </group>
    );
  ...
  // ── Diamond — faceted crystal shell + facet pinpoints
  if (skinId === "diamond") {
    return (
      <>
  ...
  if (skinId === "radioactive") {
    const veins = [
      [-0.28, 0.62, 0.28], [0.28, 0.58, 0.30], [-0.20, 0.45, 0.42],
      [0.22, 0.42, 0.40], [-0.34, 0.55, -0.10], [0.34, 0.50, -0.05],
      [0.0, 0.78, -0.35], [-0.16, 1.12, 0.78], [0.16, 1.16, 0.78],
    ];
    return (
      <group>
        {veins.map((p, i) => (
          <mesh key={i} position={p} scale={[0.05, 0.012, 0.10]} rotation={[0, i * 0.7, i * 0.3]}>
            <sphereGeometry args={[1, 10, 6]} />
            <meshStandardMaterial
              color="#C9FF6E"
              emissive="#7FFF00"
  ...
  // ── Zombie — torn flesh patches (darker green bumps)
  if (skinId === "zombie") {
    const tears = [
      [-0.30, 0.65, 0.28], [0.32, 0.55, 0.30], [-0.18, 0.42, 0.42],
      [0.20, 0.40, 0.40], [-0.32, 0.48, -0.08], [0.0, 0.78, -0.35],
      [-0.16, 1.10, 0.78], [0.16, 1.14, 0.78],
    ];
    return (
      <>
        {tears.map((p, i) => (
          <mesh key={i} position={p} scale={[0.07, 0.04, 0.07]}>
            <sphereGeometry args={[1, 8, 6]} />
            <meshStandardMaterial color="#3A4B1F" roughness={0.95} />
          </mesh>
        ))}
        {/* Stitches across the back */}
        <mesh position={[0, 0.78, -0.1]} scale={[0.32, 0.008, 0.008]}>
          <boxGeometry args={[1, 1, 1]} />
          <meshStandardMaterial color="#2A2A2A" />
        </mesh>
        <mesh position={[0, 0.60, -0.15]} scale={[0.28, 0.008, 0.008]} rotation={[0, 0, 0.2]}>
          <boxGeometry args={[1, 1, 1]} />
          <meshStandardMaterial color="#2A2A2A" />
        </mesh>
      </>
  ...
  // ── Cyber (robot) — armor seams + LED dots
  if (skinId === "robot") {
    return (
      <>
        {/* Vertical chest seam */}
        <mesh position={[0, 0.55, 0.42]} scale={[0.008, 0.4, 0.008]}>
          <boxGeometry args={[1, 1, 1]} />
          <meshStandardMaterial color="#1a1a1a" metalness={0.9} roughness={0.4} />
        </mesh>
        {/* Side seams */}
        <mesh position={[-0.42, 0.55, 0]} scale={[0.008, 0.35, 0.008]}>
          <boxGeometry args={[1, 1, 1]} />
          <meshStandardMaterial color="#1a1a1a" metalness={0.9} roughness={0.4} />
        </mesh>
        <mesh position={[0.42, 0.55, 0]} scale={[0.008, 0.35, 0.008]}>
        ...
        {/* LEDs */}
        {[[-0.15, 0.78, 0.42], [0.15, 0.78, 0.42], [-0.15, 0.35, 0.42], [0.15, 0.35, 0.42]].map((p, i) => (
          <mesh key={i} position={p}>
            <sphereGeometry args={[0.022, 10, 8]} />
            <meshStandardMaterial color="#00C2FF" emissive="#00C2FF" emissiveIntensity={2} toneMapped={false} />
          </mesh>
        ))}
      </>
    );
  ...
  // ── Gold — facet sheen pinpoints
  if (skinId === "gold") {
    const pts = [
      [-0.30, 0.65, 0.28], [0.30, 0.60, 0.30], [-0.20, 0.45, 0.42],
      [0.20, 0.42, 0.40], [0.0, 0.78, 0.40], [-0.16, 1.18, 0.78], [0.16, 1.18, 0.78],
    ];
    return (
      <>
        {pts.map((p, i) => (
          <mesh key={i} position={p}>
            ...
            <meshStandardMaterial color="#FFE57A" emissive="#FFC83A" emissiveIntensity={1.8} metalness={1} roughness={0.05} toneMapped={false} />
          </mesh>
        ))}
      </>
    );
  ...
  // ── Silver — subtle reflective specks
  if (skinId === "silver") {
    const pts = [
      [-0.28, 0.62, 0.30], [0.28, 0.58, 0.30], [0.0, 0.78, 0.40],
      [-0.16, 1.14, 0.78], [0.16, 1.16, 0.78],
    ];
    return (
      <>
        {pts.map((p, i) => (
          <mesh key={i} position={p}>
            ...
            <meshStandardMaterial color="#F0F4FA" emissive="#E0E5EE" emissiveIntensity={0.9} metalness={1} roughness={0.1} toneMapped={false} />
          </mesh>
        ))}
      </>
    );
  ...
  // ── Aqua (water) — water droplets clinging to body + fur wisps
  if (skinId === "water") {
    const drops = [
      [-0.30, 0.55, 0.32], [0.30, 0.62, 0.28], [-0.10, 0.40, 0.42],
      [0.18, 0.78, 0.32], [-0.16, 1.10, 0.78], [0.16, 1.16, 0.78],
    ];
    return (
      <>
        {drops.map((p, i) => (
  ...
  // ── Phantom (skeletal) — sparse white fur wisps
  if (skinId === "skeletal") {
    return (
      <>
        <FurTuft position={[-0.12, 1.42, 0.55]} rotation={[0, 0, -0.2]} color="#E8F2FF" size={0.04} />
        <FurTuft position={[0.12, 1.42, 0.55]} rotation={[0, 0, 0.2]} color="#E8F2FF" size={0.04} />
        <FurTuft position={[0, 1.45, 0.55]} rotation={[0, 0, 0]} color="#FFFFFF" size={0.045} />
      </>
    );
  }

  // ── Ethereal — wispy strands of cosmic fur
  if (skinId === "ethereal") {
    return (
      <>
        <FurTuft position={[-0.10, 1.40, 0.55]} rotation={[0, 0, -0.25]} color="#E8D5FF" size={0.05} />
        <FurTuft position={[0.10, 1.40, 0.55]} rotation={[0, 0, 0.25]} color="#E8D5FF" size={0.05} />
        <FurTuft position={[0, 1.45, 0.50]} rotation={[0.1, 0, 0]} color="#F5E8FF" size={0.06} />
        <FurTuft position={[-0.15, 0.85, -0.05]} rotation={[0.4, 0, -0.5]} color="#D9B5FF" size={0.04} />
        <FurTuft position={[0.15, 0.85, -0.05]} rotation={[0.4, 0, 0.5]} color="#D9B5FF" size={0.04} />
      </>
    );
  }

  // ── Default/Guardian — real fur tufts on top of head, ears, back, cheeks
  if (skinId === "default") {
    const furCol = "#7A4B28"; // matches default body undertone
    const furLight = "#A06A3E";
    return (
      <>
        {/* Top-of-head tufts (between the horns) */}
        <FurTuft position={[-0.08, 1.40, 0.55]} rotation={[0, 0, -0.2]} color={furLight} size={0.045} />
        <FurTuft position={[0.08, 1.40, 0.55]} rotation={[0, 0, 0.2]} color={furLight} size={0.045} />
        <FurTuft position={[0, 1.43, 0.50]} rotation={[0.1, 0, 0]} color={furCol} size={0.055} />
        {/* Cheek tufts */}
        <FurTuft position={[-0.30, 0.95, 0.55]} rotation={[0, 0, -0.6]} color={furCol} size={0.04} />
        <FurTuft position={[0.30, 0.95, 0.55]} rotation={[0, 0, 0.6]} color={furCol} size={0.04} />
        {/* Back tufts */}
        <FurTuft position={[-0.18, 0.95, -0.15]} rotation={[0.3, 0, -0.3]} color={furCol} size={0.05} />
        <FurTuft position={[0.18, 0.95, -0.15]} rotation={[0.3, 0, 0.3]} color={furCol} size={0.05} />
        <FurTuft position={[0, 0.98, -0.30]} rotation={[0.4, 0, 0]} color={furLight} size={0.055} />
        {/* Side tufts (shoulders) */}
        <FurTuft position={[-0.42, 0.70, 0.10]} rotation={[0, 0, -1.2]} color={furCol} size={0.04} />
        <FurTuft position={[0.42, 0.70, 0.10]} rotation={[0, 0, 1.2]} color={furCol} size={0.04} />
      </>
    );
  }
...
export const CosmicRunner3DScene = forwardRef(function CosmicRunner3DScene(
  ...
  // Touch swipe — bound to `document` (not the viewport div) so the
  // listeners reliably receive every touch even though React Three
  // Fiber's <Canvas> sits on top of our viewport and would otherwise
  // intercept the events.
  //
  // Why `document` + `passive: false` and not the viewport ref:
  //   The first version of this fix attached non-passive listeners to a
  //   ref on the root viewport <div>. That works for plain DOM, but the
  //   R3F Canvas inside the viewport receives the touchstart on its own
  //   element first and doesn't bubble in a way the ref handler sees
  //   reliably on every browser. The net effect was: no swipe registered
  //   in-game AND the page still scrolled (because `touch-action: none`
  //   on our ref div didn't influence the Canvas's gesture). Binding to
  //   `document` sidesteps that — every touch is observable here.
  //
  // We only call preventDefault during gameplay so the rest of the site
  // (admin panels, modals, the betting page, etc.) keeps native scroll.
  useEffect(() => {
    ...
    const onMove = (e) => {
      // While running, swallow every touchmove so the browser can't
      // scroll the page or pinch-zoom the viewport.
    ...
    // `passive: false` is mandatory for preventDefault() to actually work
    // on touchmove. Modern Chrome/Safari treat touchstart/touchmove on
    // document/window as passive by default unless explicitly opted out.
  ...
  // While the game is playing, lock body scrolling as a belt-and-suspenders
  // measure. Even if a stray touch reaches an ancestor scroll container,
  // it can't move because the body itself is non-scrollable.
  useEffect(() => {
    if (!playing) return;
  ...
  return (
    <div
      ...
      style={{
        ...
        // pinch-zoom on the viewport itself. Document-level listeners
        // above also call preventDefault() during gameplay; this is
        // belt-and-suspenders against UAs that ignore the JS path.
        touchAction: "none",
