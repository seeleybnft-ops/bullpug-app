...
@api_router.get("/forum/post/{post_id}")
...
@api_router.post("/forum/like/{item_type}/{item_id}")
async def toggle_like(item_type: str, item_id: str, wallet_address: str):
...
async def get_forum_categories():
    ...
    return {
        "categories": [
            {"id": "general", "name": "General Discussion", "description": "Talk about anything Bullpug related"},
            {"id": "trading", "name": "Trading & Analysis", "description": "Share trading strategies and market analysis"},
            {"id": "betting", "name": "Betting Arena", "description": "Discuss P2P betting and strategies"},
            {"id": "memes", "name": "Memes & Fun", "description": "Share memes and have fun"},
            {"id": "support", "name": "Help & Support", "description": "Get help from the community"},
...
@api_router.post("/email/test")
async def test_email_config():
    """Test if email is configured (admin only)"""
    if sendgrid_api_key:
        return {"configured": True, "sender": sender_email}
    return {"configured": False, "message": "SENDGRID_API_KEY not set"}
